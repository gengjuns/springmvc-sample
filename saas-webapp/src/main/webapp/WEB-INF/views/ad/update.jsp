...
<layout:page>
	<layout:update modelAttribute="aDConfig" titleCode="edit_ad" multipart="true">
		<layout:row>
			<field:input  path="name" labelCode="ad_name" placeholder="Name" required="true" />
		</layout:row>
		<layout:row>
			<field:input  path="host" labelCode="ad_host" placeholder="Host" required="true"/>
		</layout:row>
		<layout:row>
			<field:input  path="port" labelCode="ad_port" placeholder="Port" required="true" />
		</layout:row>
		<layout:row>
			<field:input  path="loginDN" labelCode="ad_logindn" placeholder="LoginDN" required="true"/>
		</layout:row>
		<layout:row>
			<field:password  path="password" labelCode="ad_password" placeholder="Password" required="true" showPassword="true"/>
		</layout:row>
		<layout:row>
			<field:input  path="searchDN" labelCode="ad_searchdn" placeholder="SearchDN" required="true"/>
		</layout:row>
		<layout:row>
			<div id="condition">
				...
				<div id="relevance" class="control-group">
					<c:forEach items="${aDConfig.customADs}" var="customAD" varStatus="i">
						<div class="controls-row" style="margin-top:10px;margin-bottom:5px" id="filter${i.index }">
							<div style='float:left;margin-top:5px;;margin-left:80px;margin-right:10px;'>Custom Field</div>
							<div style="float:left;">
								<form:select items="${customfields}" itemLabel="key" itemValue="value" path="customADs[${i.index }].customField" data-type="select"></form:select>
							</div>
							<div style='float:left;margin-top:5px;margin-left:10px;margin-right:10px;'>AD Field</div>
							<div style="float:left;">
								<form:select items="${adfields}" itemLabel="key" itemValue="value" path="customADs[${i.index }].adField" data-type="select"></form:select>
							</div>
							<div style="float:left;"class="span1">
								<button type="button" value=""  id="btnRemove" class="btn"><span class="icon-minus"></span></button>
							</div>	
						</div>
					</c:forEach>
				</div>
...
<script type="text/javascript">
	...
	function formatRowStr(i) {
		var customField = 'customADs[' + i
				+ '].customField';
		var adField = 'customADs[' + i
				+ '].adField';
