...
<layout:column width="${width }" path="${path }" labelCode="${labelCode}" label="${label}" helpText="${helpText}" required="${required}">
	...
	<form:input data-name="${dataName }" path="${path }" id="${styleId }" placeholder="${placeholder }" onblur="${onblur }" onchange="${onchange }" onclick="${onclick }" onfocus="${onfocus }"/><span class="add-on">%</span>
     </div>
</layout:column>
