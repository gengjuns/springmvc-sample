...
<layout:column width="${width}" path="${path}" labelCode="${labelCode}" label="${label}" helpText="${helpText }" required="${required}">
	<form:textarea data-name="${dataName }" path="${path}" id="${styleId}" placeholder="${placeholder}" rows="${var_row}" onblur="${onblur }" onchange="${onchange }" onclick="${onclick }" onfocus="${onfocus }" maxlength="${maxlength}"/>
</layout:column>